...
**Claude Code** 是 Anthropic 官方提供的命令行工具，可直接在终端中进行代码分析和生成。本文介绍如何在 WSL2 环境下安装 Claude
Code，并使用 **CC Switch** 统一管理主流模型 API。  
...
使用 CC Switch 可以方便地在多个模型 (如 Claude 2、Claude Instant 100k、Gemini Pro 等) 之间切换，统一管理 API Key
和请求参数。  
...
1. 点击左上角齿轮图标进入设置。
2. 选择高级设置，展开配置文件目录。
3. 将 Claude Code 配置目录路径修改为 WSL 虚拟机中的实际路径 (通常是
   `\\wsl.localhost\Ubuntu-xx.xx\home\[username]\.claude`)。
...
进入 CC Switch 后，选择 Claude 配置，点击右上角黄色的 + 号，选择对应的模型提供商 (如 Zhipu GLM)，输入从提供商获取到的 API Key 后保存即可。

- **Opus (最强推理)** → 最高阶、长上下文、强逻辑模型
- **Sonnet (均衡主力)** → 中高端、日常编码/对话主力
- **Haiku (快速轻量)** → 轻量、快响应、低成本模型
- **主模型 (Default)** → 你日常最常用、优先级最高的那一个

## GLM 模型映射推荐配置 (按“能力+成本”平衡)
|        档位        | 推荐模型                  | 定位与用途                   |
|:----------------:|:----------------------|:------------------------|
| **主模型（Default）** | **GLM-5.1**           | 最新最强，日常编码、复杂逻辑、长上下文优先用它 |
|  **Opus 默认模型**   | **GLM-5.1 / GLM-4.7** | 复杂推理、深度代码分析、长文档、高要求任务   |
| **Sonnet 默认模型**  | **GLM-4.7**           | 均衡主力：日常编码、接口、业务逻辑、中等复杂度 |
|  **Haiku 默认模型**  | **GLM-4.5-Air**       | 轻量快速：简单脚本、查询、补全、低延迟、低成本 |
