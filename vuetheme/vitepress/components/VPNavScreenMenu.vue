...
<template>
  <nav v-if="config.nav" class="VPNavScreenMenu">
    <template v-for="item in config.nav" :key="item.text">
      <VPNavScreenMenuLink v-if="'link' in item" :text="item.text" :badge="item.badge" :link="item.link" />
      <VPNavScreenMenuGroup v-else :text="item.text || ''" :badge="item.badge" :items="item.items" />
    </template>
  </nav>
</template>
